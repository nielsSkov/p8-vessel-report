...
The correction data from the basestation consists of different message types, varying in length.
...
To ensure the correct package sizes is send, an initiazation sequence is run on the server at startup, which searches for the init sequence, finds the packet size, then reads that amount of bytes, until a package is received with that length, followed by another start sequence.
