\section{\texorpdfstring{$\mathcal{H}_\infty$} Design}
...
The matrix $\vec{A}_1$ describes the dynamics of the system states and all the other added states, which account for the references and disturbances. Its value can be seen in \autoref{app:matrices} and its structure is \fxnote{make appendix for all matrices} \fxnote{mention A and B matrices for integral, wave, wind etc comes from weighting functions}
\begin{flalign}
	\label{eq:A1}
	\vec{A}_1 &=
	\begin{bmatrix}
...
\begin{flalign}
	...
	\end{bmatrix}\ . \nonumber
\end{flalign}
Where the weighting submatrices values have been chosen as \fxnote{how we choose the weights}.The value of the $\vec{C}_1$ matrix can be seen in \autoref{app:matrices}.
