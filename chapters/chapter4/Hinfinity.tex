...
\begin{where}
  ...
  \va{\vec{D}_{21}}{is the direct feedforward matrix from $\vec{w}$ to $\vec{y}$}{}
  \va{\vec{D}_{22}}{is the direct feedforward matrix from $\vec{u}$ to $\vec{y}$}{}
\end{where}
...
\begin{flalign}
	W_{\psi_\mathrm{ref}} &= \frac{1}{s+12}\ ,\nonumber \\
...
\begin{minipage}{0.49\linewidth}
\begin{flalign}
	W_{\dot{x}_\mathrm{b,ref}} &= \frac{1}{s+12}\ ,\nonumber \\
...
The uncontrolled inputs include the references to be tracked, the input disturbances, and the measurement noises. The size of this vector depends on the amount of reference signals, the input disturbances considered (wind, current and waves) and the amount of measured outputs, as these are normally affected by noise. The references for the inner controller are two, the heading, $\psi$, and the translational speed along the $x_\mathrm{b}$ direction. The input disturbances are coming from wind and from waves. These two elements potentially generate both a force along the $x_\mathrm{b}$ and a torque in $\psi$. The noise vector affects measured outputs considered for the inner controller, which are the outputs, $\psi$ and $\dot{x}_\mathrm{b}$. The uncontrolled input vector is formed as
\begin{flalign}
	\vec{w(t)}= 
	\begin{bmatrix}
...
\end{flalign} 
\begin{where}
	\va{F_\mathrm{wind/wave}}{is the force of the wind/waves along the $x_\mathrm{b}$ direction}{N}
	\va{\tau_\mathrm{wind/wave}}{is the torque of the wind/waves in $\psi$}{N \cdot m}
...
\begin{flalign}
	\vec{W}_\mathrm{x} = diag(2,3,2)\ ,\vec{W}_\mathrm{I}=d&iag(5,2)\ ,\vec{W}_\mathrm{wind}=diag(1,1)\ ,  \\
...
\begin{figure}[H]
    ...
    \caption{Final block diagram of the $\mathcal{H}_\infty$ controller design. \fxnote{xinf in states}}
