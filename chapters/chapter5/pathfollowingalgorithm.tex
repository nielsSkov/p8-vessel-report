...
\begin{where}
	...
	\va{[x_\mathrm{k},y_\mathrm{k}]}{is the current waypoint followed in the path}{}
	\va{[x_\mathrm{k+1},y_\mathrm{k+1}]}{is the following waypoint followed in the path}{}
\end{where}
%
\begin{figure}[H]
...
\end{figure}
With this approach, the vessel will always try to move forward in the path although a waypoint position has not being precisely attained. % This avoids situations like the one shown in \autoref{fig:goingbackapproach} where the vessel turns around in order to hit the waypoint precisely.
